    ...
    async function loginWithGoogle(authCode) {
        try {
            ...
            if (response?.status === 200) {
                ...
                console.error('Google login failed (backend response error):', response?.data?.message || 'Unknown error');
                // TODO: Show a user-friendly error notification
                return false; // Login failed
            }
        } catch (error) {
            console.error('Google login request error (backend communication issue):', error);
             if (error.response) {
