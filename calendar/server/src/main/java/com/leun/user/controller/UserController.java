...
//    }
    ...
    @PatchMapping("/user/setting/ai-schedule-days")
    public ResponseEntity<UserSettingDto.Response> updateAiScheduleDays(
        @AuthenticationPrincipal UserDetails userDetails,
        @RequestBody UserSettingDto.Request.AiScheduleDays request) throws Exception {

        UserSettingDto.Response response =
            userService.updateAiScheduleDays(userDetails.getUsername(), request.getAiScheduleDays());

        return ResponseEntity.ok(response);
    }

    @PatchMapping("/user/setting/ai-custom-rules")
    public ResponseEntity<UserSettingDto.Response> updateAiCustomRules(
        @AuthenticationPrincipal UserDetails userDetails,
        @RequestBody UserSettingDto.Request.AiCustomRules request) throws Exception {

        UserSettingDto.Response response =
            userService.updateAiCustomRules(userDetails.getUsername(), request.getAiCustomRules());
