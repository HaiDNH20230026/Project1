...
public class UserService {
    ...
    @Transactional
    public UserSettingDto.Response updateAiScheduleDays(String email, Integer days) throws Exception {
        // Validate: min 1, max 14
        if (days == null || days < 1) days = 1;
        if (days > 14) days = 14;
        
        userSettingRepository.updateAiScheduleDays(email, days);
        return userSettingRepository.findUserSettingByEmail(email);
    }

    public Integer getAiScheduleDays(String email) {
        Integer days = userSettingRepository.findAiScheduleDaysByEmail(email);
        return days != null ? days : 4; // default 4
    }

    @Transactional
    public UserSettingDto.Response updateAiCustomRules(String email, String rules) throws Exception {
        userSettingRepository.updateAiCustomRules(email, rules != null ? rules : "");
        return userSettingRepository.findUserSettingByEmail(email);
    }

    public String getAiCustomRules(String email) {
